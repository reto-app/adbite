...
## Phase 6: Channel Store — see docs/ROKU-STORE.md

**Status 2026-09-17: the code blockers are cleared.** The screensaver
override is gone (Roku prohibits it outright, so a shop turns the TV's own
screensaver off instead), Back behaves the way certification asks, the
540x405 store poster is generated by `tools/make-art.py`, and the pairing
screen offers a whole working sample board on OK so a reviewer who has no
account can still see the product. What is left needs a person: a signing
key generated on a device, the listing, screenshots from a real TV, and the
submission notes. `docs/ROKU-STORE.md` is the checklist.
