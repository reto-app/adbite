...
function fingerprint(spots) {
  return createHash('sha256').update(spots.map((s) => s.creatives.storage_path).join('|')).digest('hex').slice(0, 32);
}
...
async function buildReel(shopId, spots, dir) {
  const inputs = [];
  const parts = [];
  let index = 0;
  for (const spot of spots) {
    const creative = spot.creatives;
    const local = join(dir, `clip${index}.${creative.kind === 'video' ? 'mp4' : 'img'}`);
    await download(creative.storage_path, local);
    const seconds = creative.kind === 'video' ? Math.max(2, Number(creative.seconds) || 15) : 12;
    if (creative.kind === 'video') {
      inputs.push('-i', local);
    } else {
...
async function reelFor(shopId, dir) {
  ...
  log(`reel for ${shopId}: ${spots.length} spot(s), ${seconds.toFixed(0)}s, ${(bytes / 1048576).toFixed(1)} MB`);
  return true;
}

/** Shops with a second screen, which are the only ones that need a reel. */
async function shopsWantingReels() {
  const { data } = await db.from('devices').select('shop_id').eq('screen', 'reel').not('shop_id', 'is', null);
